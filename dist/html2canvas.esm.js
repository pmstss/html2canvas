...
const isWhiteSpace = (codePoint) => codePoint === LINE_FEED || codePoint === CHARACTER_TABULATION || codePoint === SPACE$1;
...
const isDimensionToken = (token) => token.type === TokenType.DIMENSION_TOKEN;
const isNumberToken = (token) => token.type === TokenType.NUMBER_TOKEN;
const isIdentToken = (token) => token.type === TokenType.IDENT_TOKEN;
const isStringToken = (token) => token.type === TokenType.STRING_TOKEN;
const isIdentWithValue = (token, value) => isIdentToken(token) && token.value === value;
const nonWhiteSpace = (token) => token.type !== TokenType.WHITESPACE_TOKEN;
const nonFunctionArgSeparator = (token) => token.type !== TokenType.WHITESPACE_TOKEN && token.type !== TokenType.COMMA_TOKEN;
...
const isLength = (token) => token.type === TokenType.NUMBER_TOKEN || token.type === TokenType.DIMENSION_TOKEN;

const isLengthPercentage = (token) => token.type === TokenType.PERCENTAGE_TOKEN || isLength(token);
...
const parseColorStop = (args) => {
    ...
    const stop = args[1];
    return stop && isLengthPercentage(stop) ? { color: color$1, stop } : { color: color$1, stop: null };
};
...
const fontFamily = {
    ...
    type: PropertyDescriptorParsingType.LIST,
    parse: (tokens) => {
        return tokens.filter(isStringToken$1).map(token => token.value);
    }
};
const isStringToken$1 = (token) => token.type === TokenType.STRING_TOKEN || token.type === TokenType.IDENT_TOKEN;
...
const transform$1 = (text, transform) => {
    switch (transform) {
...
class AnchorElementContainer extends ElementContainer {
    constructor(element) {
        super(element);
    ...
    for (let childNode = node.firstChild, nextNode; childNode; childNode = nextNode) {
        ...
        if (isTextNode(childNode) && childNode.data.trim().length > 0) {
            ...
            */
            ...
            const styles = new CSSParsedDeclaration(window.getComputedStyle(node, null));
            /*
...
const isHTMLElementNode = (node) => typeof node.style !== 'undefined';
...
const isAnchorElement = (node) => node.tagName === 'A';
...
const isBezierCurve = (path) => path.type === PathType.BEZIER_CURVE;
...
const isTransformEffect = (effect) => effect.type === 0 /* TRANSFORM */;
const isClipEffect = (effect) => effect.type === 1 /* CLIP */;
...
class ForeignObjectRenderer {
    ...
    async render(element) {
        ...
        const img = await loadSerializedSVG$1(svg);
        if (this.options.backgroundColor) {
            this.ctx.fillStyle = asString(this.options.backgroundColor);
...
const loadSerializedSVG$1 = (svg) => new Promise((resolve, reject) => {
    const img = new Image();
    img.onload = () => {
        resolve(img);
    };
...
const parseColor$1 = (value) => color.parse(Parser.create(value).parseComponentValue());
...
const renderElement = async (element, opts) => {
    ...
    const defaultBackgroundColor = typeof bgColor === 'string' ? parseColor$1(bgColor) : bgColor === null ? COLORS.TRANSPARENT : 0xffffffff;
