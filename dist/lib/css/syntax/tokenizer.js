...
const stringToNumber = (codePoints) => {
    ...
    const int = integers.length ? parseInt(css_line_break_1.fromCodePoint(...integers), 10) : 0;
    ...
    const frac = fracd ? parseInt(css_line_break_1.fromCodePoint(...fraction), 10) : 0;
    ...
    const exp = exponent.length ? parseInt(css_line_break_1.fromCodePoint(...exponent), 10) : 0;
...
class Tokenizer {
    ...
    consumeUnicodeRangeToken() {
        ...
        if (questionMarks) {
            const start = parseInt(css_line_break_1.fromCodePoint(...digits.map(digit => (digit === QUESTION_MARK ? ZERO : digit))), 16);
            const end = parseInt(css_line_break_1.fromCodePoint(...digits.map(digit => (digit === QUESTION_MARK ? F : digit))), 16);
            return { type: TokenType.UNICODE_RANGE_TOKEN, start, end };
        }
        const start = parseInt(css_line_break_1.fromCodePoint(...digits), 16);
        if (this.peekCodePoint(0) === HYPHEN_MINUS && isHex(this.peekCodePoint(1))) {
            ...
            const end = parseInt(css_line_break_1.fromCodePoint(...endDigits), 16);
            return { type: TokenType.UNICODE_RANGE_TOKEN, start, end };
        }
        else {
    ...
    consumeUrlToken() {
        ...
        while (true) {
            ...
            if (codePoint === EOF || codePoint === RIGHT_PARENTHESIS) {
                return { type: TokenType.URL_TOKEN, value: css_line_break_1.fromCodePoint(...value) };
            }
    ...
    consumeStringSlice(count) {
        ...
        while (count > 0) {
            ...
            value += css_line_break_1.fromCodePoint(...this._value.splice(0, amount));
            count -= amount;
        }
        this._value.shift();
        return value;
    ...
    consumeEscapedCodePoint() {
        const codePoint = this.consumeCodePoint();
        if (isHex(codePoint)) {
            let hex = css_line_break_1.fromCodePoint(codePoint);
